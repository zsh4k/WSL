...
// hex chars. A determinstic subname keeps the per-share guest-side path
            ...
            try
            {
                ...
                else if (message->MessageType == LxInitMessageRemountVirtioFsDevice)
                {
                    ...
                    const auto result = wil::ResultFromException([this, span, message, &newTag, &newSubname, &source]() {
                        ...
                        //
                        // SubnameOffset was appended to the remount message; older clients
                        // omit it. Size-gate AND bounds-check before reading so a smaller
                        // old-init payload whose appended slot overlaps garbage falls back
                        // to legacy-tag lookup instead of dereferencing a bogus offset.
                        //
