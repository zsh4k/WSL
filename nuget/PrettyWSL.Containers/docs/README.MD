...
target_link_libraries(my_app PRIVATE PrettyWSL.Containers::SDK)
```
...
- Adds `PrettyWSL.Containers.winmd` as a reference so C++/WinRT generates projection headers
...
```cpp
#include <winrt/PrettyWSL.Containers.h>
